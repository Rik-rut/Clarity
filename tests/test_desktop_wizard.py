...
def test_an_environment_that_still_works_is_adopted_rather_than_deleted(
    ...
    monkeypatch.setattr(wizard.runtime_steps, "_venv_imports_the_stack", lambda env_dir: True)
    monkeypatch.setattr(wizard.runtime_steps, "run_streamed", fake_uv(base, calls))
    monkeypatch.setattr(wizard, "detect_gpu", lambda: GpuInfo())
    ...
    assert any("Reusing the environment from an earlier run" in line for line in snapshot["lines"])
    assert get_json(f"{server}/api/setup/status")["state"]["steps"]["runtime"] == "done"
...
def test_an_unusable_env_that_cannot_be_deleted_names_the_running_process(
    ...
    monkeypatch.setattr(wizard.runtime_steps.shutil, "rmtree", refuse)

    with pytest.raises(SetupError) as raised:
        wizard.runtime_steps.create_venv(base, ProgressTracker())
...
def test_the_probe_asks_the_interpreter_itself(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(wizard.runtime_steps.subprocess, "run", fake_run)
    assert wizard.runtime_steps._venv_imports_the_stack(tmp_path / "env") is True
    assert seen[0][0] == str(python)
    assert wizard.runtime_steps.VERIFY_IMPORTS in " ".join(seen[0])


def test_the_probe_is_false_when_there_is_nothing_to_ask(tmp_path: Path) -> None:
    assert wizard.runtime_steps._venv_imports_the_stack(tmp_path / "missing") is False
...
def test_dependency_install_refuses_an_env_without_an_interpreter(base: SetupContext) -> None:
    ...
    with pytest.raises(SetupError, match="no interpreter"):
        wizard.runtime_steps.install_dependencies(base, ProgressTracker(), "cpu")
...
def test_windows_access_denied_is_retried_once_and_explains_itself(
    ...
    slept: list[float] = []
    monkeypatch.setattr(wizard.runtime_steps, "run_streamed", deny)
    monkeypatch.setattr(wizard.runtime_steps.time, "sleep", lambda seconds: slept.append(seconds))
