...
# Check if uv is in PATH or ~/.local/bin
...
if [ "${1:-}" = "--setup" ] || [ "${1:-}" = "-s" ] || [ "${1:-}" = "setup" ]; then
    DO_SETUP=1
elif ! command -v uv >/dev/null 2>&1 || [ ! -f ".venv/bin/python" ]; then
    DO_SETUP=1
fi
...
if [ "$DO_SETUP" -eq 1 ]; then
    ...
    if command -v uv >/dev/null 2>&1; then
        ...
        if ! command -v uv >/dev/null 2>&1; then
            ...
            echo "  Close this terminal, open a NEW one, and run:  bash run.sh"
            fail
        fi
    fi
    ...
    if [ "${1:-}" = "--setup" ] || [ "${1:-}" = "-s" ] || [ "${1:-}" = "setup" ]; then
        echo "Setup finished. Run ./run.sh to start Clarity anytime!"
        exit 0
    fi
fi
