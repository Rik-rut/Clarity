...
class InferenceCore:
    ...
    def __init__(self,
        ...
        if isinstance(network, str):
            # ponytail: upstream auto-downloaded via huggingface_hub
            # (PyTorchModelHubMixin.from_pretrained), flattened out. Instantiate
            # MatAnyone2 directly (get_matanyone2_model + torch.load) instead.
            raise TypeError(
                "string network ids are unsupported in the vendored build; "
                "pass a MatAnyone2 instance")
        network.to(device)
        network.eval()
