...
from ..model.big_modules import PixelEncoder, UncertPred, KeyProjection, MaskEncoder, PixelFeatureFuser, MaskDecoder
from ..model.aux_modules import AuxComputer
from ..model.utils.memory_utils import get_affinity, readout
...
class MatAnyone2(nn.Module):
    # ponytail: upstream inherits huggingface_hub's PyTorchModelHubMixin
    # (from_pretrained/save_pretrained). Flattened out for the vendored
    # inference-only build; load checkpoints via load_weights instead.
    # Restore the mixin if HF-hub model loading is ever needed.
