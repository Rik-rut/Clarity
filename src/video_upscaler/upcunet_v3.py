# Vendored from bilibili/ailab Real-CUGAN (https://github.com/bilibili/ailab/tree/main/Real-CUGAN),
# file upcunet_v3.py. MIT License, Copyright (c) 2022 bilibili.
# Original source unmodified except where noted.
'''
cache_mode:
0:ä½¿ç”¨cacheç¼“å­˜å¿…è¦å‚æ•°
1:ä½¿ç”¨cacheç¼“å­˜å¿…è¦å‚æ•°ï¼Œå¯¹cacheè¿›è¡Œ8bité‡åŒ–èŠ‚çœæ˜¾å­˜ï¼Œå¸¦æ¥å°è®¸å»¶æ—¶å¢žé•¿
2:ä¸ä½¿ç”¨cacheï¼Œè€—æ—¶çº¦ä¸ºmode0çš„2å€ï¼Œä½†æ˜¯æ˜¾å­˜ä¸å—è¾“å…¥å›¾åƒåˆ†è¾¨çŽ‡é™åˆ¶ï¼Œtile_modeå¡«å¾—å¤Ÿå¤§ï¼Œ1.5Gæ˜¾å­˜å¯è¶…ä»»æ„æ¯”ä¾‹
'''
import torch,pdb
...
def q(inp,cache_mode):
    ...
    delta = maxx - minn
    if(cache_mode==2):
        return ((inp-minn)/delta*255).round().byte().cpu(),delta,minn,inp.device#å¤§æ¦‚3å€å»¶æ—¶#å¤ªæ…¢äº†ï¼Œå±è”½è¯¥æ¨¡å¼
    elif(cache_mode==1):
        return ((inp-minn)/delta*255).round().byte(),delta,minn,inp.device#ä¸ç”¨CPUè½¬ç§»
def dq(inp,if_half,cache_mode,delta,minn,device):
    if(cache_mode==2):
        ...
        if(if_half==True):return inp.half()/255*delta+minn#ä¸ç”¨CPUè½¬ç§»
...
class UNet2(nn.Module):
    ...
    def forward_a(self, x):#conv234ç»“å°¾æœ‰se
        x1 = self.conv1(x)
        x2 = self.conv1_down(x1)
        x1 = F.pad(x1, (-16, -16, -16, -16))
        x2 = F.leaky_relu(x2, 0.1, inplace=True)
    ...
    def forward_b(self, x2):  # conv234ç»“å°¾æœ‰se
        x3 = self.conv2_down(x2)
        x2 = F.pad(x2, (-4, -4, -4, -4))
        x3 = F.leaky_relu(x3, 0.1, inplace=True)
    ...
    def forward_c(self, x2,x3):  # conv234ç»“å°¾æœ‰se
        x3 = self.conv3_up(x3)
        x3 = F.leaky_relu(x3, 0.1, inplace=True)
        x4 = self.conv4.conv(x2 + x3)
        return x4

    def forward_d(self, x1,x4):  # conv234ç»“å°¾æœ‰se
        x4 = self.conv4_up(x4)
        x4 = F.leaky_relu(x4, 0.1, inplace=True)
        x5 = self.conv5(x1 + x4)
        x5 = F.leaky_relu(x5, 0.1, inplace=True)
...
class UpCunet2x(nn.Module):
    ...
    def forward(self, x,tile_mode,cache_mode,alpha,pro):
        ...
        if(tile_mode==0):#ä¸tile
            ph = ((h0 - 1) // 2 + 1) * 2
            pw = ((w0 - 1) // 2 + 1) * 2
            x = F.pad(x, (18, 18 + pw - w0, 18, 18 + ph - h0), 'reflect')  # éœ€è¦ä¿è¯è¢«2æ•´é™¤
        ...
        elif(tile_mode==1):# å¯¹é•¿è¾¹å‡åŠ
            if(w0>=h0):
                crop_size_w=((w0-1)//4*4+4)//2#å‡åŠåŽèƒ½è¢«2æ•´é™¤ï¼Œæ‰€ä»¥è¦å…ˆè¢«4æ•´é™¤
                crop_size_h=(h0-1)//2*2+2#èƒ½è¢«2æ•´é™¤
            else:
                crop_size_h=((h0-1)//4*4+4)//2#å‡åŠåŽèƒ½è¢«2æ•´é™¤ï¼Œæ‰€ä»¥è¦å…ˆè¢«4æ•´é™¤
                crop_size_w=(w0-1)//2*2+2#èƒ½è¢«2æ•´é™¤
            crop_size=(crop_size_h,crop_size_w)
        elif(tile_mode>=2):
            tile_mode=min(min(h0,w0)//128,int(tile_mode))#æœ€å°çŸ­è¾¹ä¸º128*128
            t2=tile_mode*2
            crop_size=(((h0-1)//t2*t2+t2)//tile_mode,((w0-1)//t2*t2+t2)//tile_mode)
        else:
            raise ValueError("tile_mode config error")
        ...
        for i in range(0,h-36,crop_size[0]):
            for j in range(0,w-36,crop_size[1]):
                ...
                del tmp_dict[i][j]
                x = torch.add(x0, x)#x0æ˜¯unet2çš„æœ€ç»ˆè¾“å‡º
                if(pro):
    ...
    def forward_fast_rough(self, x,tile_mode,alpha,pro):
        ...
        if(tile_mode<3):return self.forward(x,tile_mode,1,alpha,pro)#è‡³å°‘åˆ‡æˆ3x3
        elif(tile_mode>=3):
            tile_mode=min(min(h0,w0)//128,int(tile_mode))#æœ€å°çŸ­è¾¹ä¸º128*128
...
class UpCunet3x(nn.Module):
    ...
    def forward(self, x,tile_mode,cache_mode,alpha,pro):
        ...
        if(tile_mode==0):#ä¸tile
            ph = ((h0 - 1) // 4 + 1) * 4
            pw = ((w0 - 1) // 4 + 1) * 4
            x = F.pad(x, (14, 14 + pw - w0, 14, 14 + ph - h0), 'reflect')  # éœ€è¦ä¿è¯è¢«2æ•´é™¤
            x = self.unet1.forward(x)
            x0 = self.unet2.forward(x,alpha)
            x = F.pad(x, (-20, -20, -20, -20))
            x = torch.add(x0, x)
            ...
            if(w0>=h0):
                crop_size_w=((w0-1)//8*8+8)//2#å‡åŠåŽèƒ½è¢«2æ•´é™¤ï¼Œæ‰€ä»¥è¦å…ˆè¢«4æ•´é™¤
                crop_size_h=(h0-1)//4*4+4#èƒ½è¢«2æ•´é™¤
            else:
                crop_size_h=((h0-1)//8*8+8)//2#å‡åŠåŽèƒ½è¢«2æ•´é™¤ï¼Œæ‰€ä»¥è¦å…ˆè¢«4æ•´é™¤
                crop_size_w=(w0-1)//4*4+4#èƒ½è¢«2æ•´é™¤
            crop_size=(crop_size_h,crop_size_w)
        elif (tile_mode >= 2):
            tile_mode=min(min(h0,w0)//128,int(tile_mode))#æœ€å°çŸ­è¾¹ä¸º128*128
            t4 = tile_mode * 4
...
class UpCunet4x(nn.Module):
    ...
    def forward(self, x,tile_mode,cache_mode,alpha,pro):
        ...
        if(tile_mode==0):#ä¸tile
            ph = ((h0 - 1) // 2 + 1) * 2
            pw = ((w0 - 1) // 2 + 1) * 2
            x = F.pad(x, (19, 19 + pw - w0, 19, 19 + ph - h0), 'reflect')  # éœ€è¦ä¿è¯è¢«2æ•´é™¤
            x = self.unet1.forward(x)
            x0 = self.unet2.forward(x,alpha)
...
class RealWaifuUpScaler(object):
    ...
    def np2tensor(self, np_frame):
        transposed = np.ascontiguousarray(np.transpose(np_frame, (2, 0, 1)))
        if self.pro:
            if not self.half:
                return torch.from_numpy(transposed).unsqueeze(0).to(self.device).float() / (255 / 0.7) + 0.15
            else:
                return torch.from_numpy(transposed).unsqueeze(0).to(self.device).half() / (255 / 0.7) + 0.15
        else:
            if not self.half:
                return torch.from_numpy(transposed).unsqueeze(0).to(self.device).float() / 255
            else:
                return torch.from_numpy(transposed).unsqueeze(0).to(self.device).half() / 255
            ...
            for cache_mode in [0,1,2,3]:
                for alpha in [1]:
                    ...
                    for name in os.listdir(input_dir):
                        ...
                        #æ”¯æŒä¸­æ–‡è·¯å¾„
                        #os.link(inp_path, tmp_path)#winç”¨ç¡¬é“¾æŽ¥
                        os.symlink(inp_path, tmp_path)#linuxç”¨è½¯é“¾æŽ¥
